...
const {
  getLatestLiveMessage,
} = require("C:\Users\sujan\OneDrive\Desktop\sarayu-web\Project\sarayu-node-backend\middlewares\mqttHandler.js");
    ...
    for (const topic of topics) {
      try {
        ...
        parentPort.postMessage({
          error: `Error processing topic ${topic}: ${error.message}`,
        });
      }
    }
...
app.use((req, res, next) => {
  ...
  res.flush = res.flush || (() => {}); // ensure flush available
  logger.info(`Request to set url ${req.url}`, {
    method: req.method,
    body: req.body,
  });
  next();
...
// start the server
const port = proceee.env.port || 5000;
